...
if (requiredCredentials.length && config.appMode !== "serverless")
	for (const credential of requiredCredentials) {
		if (process.env[credential] === undefined) {
			...
			//process.exit(1); //don't end process in v.8 upward unlinke early methodology. This allows greybox to run without database existence.
		}
	}
// for (const credential of requiredConfig) {
// 	if (boxConfig[credential as keyof typeof boxConfig] === undefined) {
// 		const errorIndex = Object.keys(ErrorOutput).length;
// 		ErrorOutput["error" + errorIndex] = `Missing required config: ${credential}`;
// 	}
// }
